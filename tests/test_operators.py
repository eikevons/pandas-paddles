...
@pytest.mark.parametrize("expr, expected", _dataframe_cases)
def test_df_operators(df, expr, expected):
    test = expr(df)
...
@pytest.mark.parametrize("expr, expected", _series_cases)
def test_series_operators(s, expr, expected):
    test = expr(s)
    assert test.to_list() == expected
